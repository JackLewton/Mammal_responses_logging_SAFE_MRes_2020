...
# LOAD DATA
##################################################
# might as well grab from the header as need the depl_dur anyway. 
# first batch
...
# DO - In the next code, I've broken out species counts
# average temp and deployment level data and then brought back together using merge. 
...
newdat7$logging <- as.factor(newdat7$logging)
newdat7$logging <- factor(newdat7$logging, levels=c('pre-logging', 'during logging', 'post-logging'))
newdat7$Species <- as.factor(newdat7$Species) 
